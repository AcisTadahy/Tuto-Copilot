      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsHTML = details.participants.length
          ? `<div class="participants"><strong>Participants:</strong><ul>${details.participants
              .map(
                (participant) =>
                  `<li><span>${participant}</span><button type="button" class="participant-remove" data-activity="${name}" data-email="${participant}" title="Unregister participant">&times;</button></li>`
              )
              .join("")}</ul></div>`
